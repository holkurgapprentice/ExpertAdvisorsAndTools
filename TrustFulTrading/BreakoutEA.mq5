...
struct RANGE_STRUCT{
   ...
   RANGE_STRUCT(): start_time(0), end_time(0), close_time(9), high(0), low(DBL_MAX), f_entry(false), f_high_breakout(false), f_low_breakout(false) {};
};
RANGE_STRUCT range;
...
bool CheckInputs() {
   ...
   if(InpRangeClose >= 1440 || (InpRangeStart+InpRangeDuration)%1440 == InpRangeClose) {
      Alert("InpRangeClose number below 0 or higher than 1440 or end_time == close_time");
      return false;   
   }
...
int CountOpenPositons() {
   ...
   for (int i=total-1; i>=0; i--) {
      ...
      if (!PositionSelectByTicket(ticket)) {Print("Failed to selet pos by ticket"); return -1;}
      ulong magicNumber;
      if (!PositionGetInteger(POSITION_MAGIC, magicNumber)) {Print("Failed to get magicnumber from position"); return -1;}
...
bool ClosePositions() {
   ...
   for (int i=total-1; i>=0; i--) {
      ...
      if (!PositionSelectByTicket(ticket)) {Print("Failed to selet pos by ticket"); return false;}
      ulong magicNumber;
      if (!PositionGetInteger(POSITION_MAGIC, magicNumber)) {Print("Failed to get magicnumber from position"); return false;}
